...
### Node.js のサポート範囲（実行時と開発時の違い）

`package.json` の `engines.node` は `>=22.5.0` です。これは `node:sqlite` が利用可能になったバージョンで、**このサーバーを実行する側の要件**です。npm から入れた CLI（`beefchicken-mcp`）はランタイム依存のみを取得するため、22.5.0 で正常に動作します。

一方、**このリポジトリで開発する側の要件は 22.13.0 以上**です。ESLint 10 が `^20.19.0 || ^22.13.0 || >=24` を要求しており、それ以前のNodeに同梱される npm（22.5.0 では npm 10.8.2）は、この依存ツリーに対して `npm ci` が `Exit handler never called!` で `node_modules/.bin/` を生成しないまま終了コード0を返します。結果として後続のコマンドが `command not found` で失敗します。

CIの `test` ジョブのマトリクスが `22.13.0` を下限としているのはこのためです。`engines` を引き上げていないのは、実行時の要件が変わっていないためです。
